# Unit Name: moneyguru.loader.native
...
class Loader(base.Loader):
    ...
    def _load(self):
        def read_transaction_element(element, info):
            ...
            try:
                info.date = datetime.strptime(attrib['date'], '%Y-%m-%d').date()
        ...
        for recurrence_element in root.getiterator('recurrence'):
            ...
            for exception_element in recurrence_element.getiterator('exception'):
                try:
                    date = datetime.strptime(exception_element.attrib['date'], '%Y-%m-%d').date()
            ...
            for change_element in recurrence_element.getiterator('change'):
                try:
                    date = datetime.strptime(change_element.attrib['date'], '%Y-%m-%d').date()
